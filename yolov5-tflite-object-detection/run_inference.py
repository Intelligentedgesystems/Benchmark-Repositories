...
#     weights = weights[0]  # Use the first weight in the list
#     backend = 'tflite'
...
#     print("Loading TFLite model from:", weights)
...
#         # Run inference
#         input_data = img.permute(0, 2, 3, 1).cpu().numpy()  # Convert (B,C,H,W) to (B,H,W,C)
...
#             s += '%gx%g ' % img.shape[2:]  # print string e.g. "320x320 "
...
#     parser.add_argument('--weights', nargs='+', type=str, default=['yolov5s_f16.tflite'], help='model.tflite path(s)')
...
#     opt.img_size *= 2 if len(opt.img_size) == 1 else 1  # expand if a single value is provided
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45):
    ...
    output = []
    for x in prediction:
        x = x[xc.squeeze()]  # Fix mask shape mismatch
        if not x.shape[0]:
            continue
        x[:, 5:] *= x[:, 4:5]
        box = x[:, :4]
...
def detect(opt):
    ...
    while True:
        ...
        img = cv2.resize(im0, (imgsz[1], imgsz[0]))
        ...
        pred[..., :4] *= np.array([imgsz[1], imgsz[0], imgsz[1], imgsz[0]])
        pred = torch.tensor(pred)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', nargs='+', type=str, default=['yolov5s_float16.tflite'], help='model.tflite path(s)')
